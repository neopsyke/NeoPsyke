...
func TestValidateLiveWithBuiltinSteps(t *testing.T) {
	cfg := DefaultConfig()
	llmConfig := t.TempDir() + "/llm.yaml"
	if err := os.WriteFile(llmConfig, []byte("mock\n"), 0o644); err != nil {
		t.Fatalf("write llm config: %v", err)
	}
	cfg.LiveEval.LLMConfigFile = llmConfig
...
func TestValidateEvalRequiresInputOrReplay(t *testing.T) {
	cfg := DefaultConfig()
	errs := Validate(cfg, "eval", &ValidationOpts{})
	if len(errs) == 0 {
		t.Fatal("expected error when neither input nor session replay is provided")
	}
}

func TestValidateOnlyMemoryLiveSmokeRequiresEnablement(t *testing.T) {
	cfg := DefaultConfig()
	llmConfig := t.TempDir() + "/llm.yaml"
	if err := os.WriteFile(llmConfig, []byte("mock\n"), 0o644); err != nil {
		t.Fatalf("write llm config: %v", err)
	...
	cfg.LiveEval.LLMConfigFile = llmConfig
	errs := Validate(cfg, "run", &ValidationOpts{Live: true, OnlyStep: "memory_live_smoke"})
	if len(errs) == 0 {
		t.Fatal("expected enablement error for memory_live_smoke")
	}
}
