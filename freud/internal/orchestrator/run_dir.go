...
func UpdateSymlinkPointer(target, linkPath string) {
...
func WritePointerFile(value, filePath string) error {
	if err := os.MkdirAll(filepath.Dir(filePath), 0o755); err != nil {
		return err
	}
	tmpFile := fmt.Sprintf("%s.tmp.%d", filePath, time.Now().UnixNano())
	if err := os.WriteFile(tmpFile, []byte(value+"\n"), 0o644); err != nil {
		return err
	}
	return os.Rename(tmpFile, filePath)
}

func WriteLatestPointers(runRoot, repoRoot, runDir string) {
	if runRoot != "" {
		_ = os.MkdirAll(runRoot, 0o755)
		UpdateSymlinkPointer(runDir, filepath.Join(runRoot, "latest"))
		_ = WritePointerFile(runDir, filepath.Join(runRoot, "latest-run.txt"))
	}

	if repoRoot == "" {
		return
	}
	if strings.EqualFold(os.Getenv("FREUD_WRITE_LOCAL_POINTERS"), "false") {
		return
	}

	localRoot := filepath.Join(repoRoot, workflowDirName)
	_ = os.MkdirAll(filepath.Join(localRoot, "logs"), 0o755)
	UpdateSymlinkPointer(runDir, filepath.Join(localRoot, "latest"))
	UpdateSymlinkPointer(filepath.Join(runDir, "logs"), filepath.Join(localRoot, "logs", "latest"))
	UpdateSymlinkPointer(filepath.Join(runDir, "artifacts"), filepath.Join(localRoot, "artifacts-latest"))
	_ = WritePointerFile(runDir, filepath.Join(localRoot, "latest-run.txt"))
}
