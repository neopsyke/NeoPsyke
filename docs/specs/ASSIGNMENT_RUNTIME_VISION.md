...
This specification defines the long-term direction for NeoPsyke's assignment
capability.
...
- what the current assignment runtime already provides
...
The current assignment runtime, which grew from the earlier objective-focused
...
All assignment is still reasoning work. NeoPsyke should not grow a zoo of
...
NeoPsyke should converge toward one assignment substrate that can express
...
### One assignment substrate

The system should converge toward one assignment runtime rather than separate
planner families for recurrent tasks, responsibilities, and workflows.

### One reasoning path

The Ego should reason about assignment through the same general action
planning machinery it uses elsewhere, with only narrow trigger-specific context
shaping where necessary.

This explicitly includes plan generation. Plans for assignment must be
...
This does not require one monolithic planner prompt for every assignment
...
The current assignment runtime already covers an important portion of this
vision.

### Durable persistence

Assignment items already have:
...
This is the right foundation for assignment continuity.

### Sequential durable plans

Assignment items already support:
...
This already resembles a simple assignment runtime for sequential work.
...
The current assignment manager already lives outside the Ego and emits work
...
This gives NeoPsyke a usable operator surface for user-visible assignment.

## Current Design Limits

The current assignment runtime is best described as durable sequential work
...
- Responsibility creation needs a stronger multi-turn intake path so the Ego
  can gather enough operating detail before committing an assignment item.
- Id impulses exist, but the current impulse path still needs an Ego-owned
  ...
  candidate back through the assignment runtime as an `id_review`.
...
- Failure handling is mostly local to retries and terminal failure states. The
  broader assignment model needs explicit stall detection, escalation, and
...
- Multi-activation testing and replay are supported by adjacent infrastructure,
  but they are not yet specified as required assignment validation semantics.
- The terminology surface is still mixed. `Goal` remains in too many places
  even though `Assignment` is the architectural umbrella and user-facing
...
- separate planner families per assignment category
- one monolithic create/update/revise planner prompt across semantically
  different assignment kinds
...
The current assignment runtime should evolve into a broader assignment
runtime
...
are different shapes of the same assignment substrate.
...
`Assignment` should remain the internal architectural umbrella, but it should
not be forced as the main end-user term.

Recommended terminology:

- **Internal umbrella:** `Durable Work`
- **Runtime/service name:** `AssignmentRuntime`
- **Gateway/interface name:** `AssignmentGateway`
...
### 1. Generalize the current runtime into a AssignmentRuntime
...
Assignment should continue to be executed by waking the Ego with a typed
...
- keep the existing L1 semantic router as the only routing layer for fresh user
  input
- let that router use an LLM call to choose the assignment target
- after routing, dispatch deterministically to one of three assignment L2
  planners:
...
they reshape the semantics of the assignment item rather than just its
lifecycle state.
...
Plan generation for assignment must move from the motor layer into the Ego.
The Ego planner should produce plan steps as part of the assignment `CREATE`
...
The assignment plan contract must preserve the full runtime step model rather
...
they are committed, whether they originate from assignment creation or from
...
preservation of the assignment step contract. The refiner can revise the plan
...
receive plan-kind and terminal-policy signals so assignment plans are not
...
generalized approval context mechanism, not an assignment-specific rendering
...
- Timeout-based cleanup may clear stale leases and stale wait registrations, but
  it should not auto-delete assignment.
...
- Interactive user turns outrank background assignment.
- Within assignment, ready resumptions and overdue obligations should outrank
...
- Id should remain an impulse source only in phase 2. Ego may use a "be useful"
  ...
  `id_review` wake against existing assignment, but Id should not create new
...
- Assignment must be tested across multiple activations, not only within a
  single Ego wake.
...
- Freud replay and session-recording infrastructure should be treated as part of
  the assignment validation story.
...
- Prefer typed activation profiles over new planner families when assignment
...
- assignment needs explicit runtime ownership and safe side-effect semantics

### Actor and event-driven runtimes

Useful pattern:

- one owner for an assignment item's runtime state
...
- simple sequential assignment can go far before general graph execution is
  necessary
...
- assignment should not replace NeoPsyke's memory system, but it should
...
Evolve the current assignment runtime into a stable assignment nucleus for:
...
- Ego-level plan generation: the Ego planner produces plan steps as part of
  the assignment `CREATE` and `REVISE_PLAN` decisions, with full access to
...
- the existing L1 semantic router remains the only routing layer for fresh
  user input; it uses an LLM call to choose the target assignment L2 planner,
  and then dispatch stays deterministic on the typed route
- the assignment L2 split may stay simple in phase 1:
...
- plan refinement: a shared `PlanRefiner` validates and improves all plans
  (assignment and Ego inline) before they are committed. It is a single
  ...
  checks pass.
- full step-contract preservation: assignment plans keep dependencies,
...
- generalized approval context: approvals carry structured context entries
  (label + content) that any action type can populate. For assignment
...
- autonomous Id creation or reshaping of assignment
...
- Plan generation happens in the Ego with full context. The runtime does not
  make LLM calls for plan generation. Plans for assignment are produced as
  part of the Ego planner's `CREATE` and `REVISE_PLAN` decisions.
- All plans (assignment and Ego inline) pass through a shared refinement step
  ...
  preserves the assignment step contract. Refinement is best-effort and falls
...
- The user sees the plan steps at approval time before the work item is created.
  ...
  assignment-specific rendering path.
...
- The feature can be considered production-stable for recurrent tasks,
  recurring checks, and simple sequential assignment before phase 2 begins.

### Phase 2: Durable Monitoring And Responsibilities

### Objective

Extend the assignment nucleus to support open-ended monitoring and
...
- responsibility-style assignment with remembered operating context
...
- `AssignmentRuntime` keeps one shared runtime substrate but persists
...
- `WorkItemKind` splits the user-facing surface into `RecurrentTask` and
  `Responsibility` while preserving `Assignment` as the internal umbrella.
- `InputIntentRouter` remains the only semantic assignment router. After it
  returns `assignment`, L2 dispatch is deterministic to generic lifecycle,
...
- Delivery policy can suppress noisy repeated summaries and surface only
  meaningful deltas.
- Id review wakes can review or advance existing assignment without bypassing
  the shared assignment model.
- Responsibility creation can gather missing operating detail across several
  turns before the final assignment `CREATE`.
- Phase-1 plan generation, refinement, approval context, and user feedback
  ...
  another assignment router layer, does not introduce separate end-to-end
...
responsibilities, and later richer assignmentflows as one family of durable
work
that:
...
AssignmentRuntime rather than being replaced by a proliferation of special
