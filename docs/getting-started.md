...
- If a **required configured cognitive-role endpoint** is unavailable (the endpoints assigned to `planner`, `superego`, `meta_reasoner`, or `memory_advisor` in the active LLM config), NeoPsyke prints a clear error and exits early.
