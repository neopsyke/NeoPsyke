...
internal class ConnectorActionPlugin(
    ...
    override suspend fun execute(action: PendingAction, context: ActionExecutionContext): ActionOutcome {
        ...
        return ActionOutcome(
            ...
            observedEvidence = descriptor.effectClass == ActionEffectClass.OBSERVE,
    ...
    private fun successEffects(effectClass: ActionEffectClass): Set<ActionEffect> =
        when (effectClass) {
            ActionEffectClass.OBSERVE ->
...
object ConnectorActionPluginLoader {
    fun load(config: AgentConfig): ConnectorPluginLoadResult {
        ...
        val catalogResult = CuratedConnectorCatalogLoader.load(Paths.get(config.connectors.curatedCatalogPath))
        val stateResult = InstalledConnectorStateLoader.load(Paths.get(config.connectors.installStateDir))
