 ...
 *
 * Generic async-operation waits are restored from persisted assignment state and
...
class WaitConditionMonitor(
    ...
    fun register(workItemId: String, stepId: String, condition: WaitCondition) {
        registrations[RegistrationKey(workItemId, stepId)] = RegistrationState(
            registration = Registration(workItemId, stepId, condition)
        )
        logger.debug { "Wait condition registered: assignment=$workItemId, step=$stepId, type=${condition.type}" }
    }

    fun unregister(workItemId: String, stepId: String) {
        registrations.remove(RegistrationKey(workItemId, stepId))
    }
    ...
    private suspend fun checkConditions() {
        ...
        for ((key, state) in snapshot) {
            ...
            if (timedOut) {
                logger.debug { "Wait condition timed out: assignment=${key.workItemId}, step=${key.stepId}" }
                registrations.remove(key)
                onConditionTimedOut(key.workItemId, key.stepId)
                continue
            }
    ...
    private fun resolveAsyncWaitIfReady(
        ...
        onConditionSatisfied(key.workItemId, key.stepId, resolution)
    }
