...
/**
 * Coroutine-based timer that fires [AssignmentCue.ScheduledWakeUp] signals
...
class TimerScheduler(
    ...
    /**
     * M1: Cron registrations — workItemId → cronExpression.
    ...
    fun register(workItemId: String, wakeAt: Instant) {
        val ms = wakeAt.toEpochMilli()
        timers.getOrPut(ms) { mutableSetOf() }.add(workItemId)
        logger.debug { "Timer registered: workItem=$workItemId, wakeAt=$wakeAt" }
    }
     ...
     *
     * @param workItemId the assignment to wake up
     * @param expression a standard 5-field cron string, e.g. `"0 9 * * *"` for daily at 09:00
     */
    fun registerCron(workItemId: String, expression: String) {
        val next = CronParser.nextAfter(expression, ZonedDateTime.now())
        if (next == null) {
            logger.warn { "Could not compute next cron time for workItem=$workItemId expr='$expression'" }
            return
        }
        cronSchedules[workItemId] = expression
        val ms = next.toInstant().toEpochMilli()
        timers.getOrPut(ms) { mutableSetOf() }.add(workItemId)
        logger.info { "Cron registered: workItem=$workItemId, expr='$expression', next=$next" }
    }

    fun cancel(workItemId: String) {
        cronSchedules.remove(workItemId)
        timers.values.forEach { it.remove(workItemId) }
    ...
    private fun checkTimers() {
        ...
        firedEntries.clear()
        for ((workItemId, scheduledAtMs) in fired) {
            logger.info { "Timer fired: workItem=$workItemId" }
            onWakeUp(workItemId, scheduledAtMs)
            // M1: Re-register next occurrence for cron schedules
            cronSchedules[workItemId]?.let { expr ->
                val next = CronParser.nextAfter(expr, ZonedDateTime.now())
                if (next != null) {
                    val nextMs = next.toInstant().toEpochMilli()
                    timers.getOrPut(nextMs) { mutableSetOf() }.add(workItemId)
                    logger.info { "Cron rescheduled: workItem=$workItemId, next=$next" }
                } else {
                    logger.warn { "Could not reschedule cron for workItem=$workItemId — removing" }
                    cronSchedules.remove(workItemId)
                }
            }
        }
