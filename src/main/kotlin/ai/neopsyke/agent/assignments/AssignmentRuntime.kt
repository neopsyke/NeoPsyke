...
private data class AssignmentRunSession(
...
class AssignmentRuntime(
    private val config: AssignmentConfig,
    private val store: WorkItemStore,
    private val planner: AssignmentPlanBuilder = DeterministicAssignmentPlanBuilder(),
    ...
    private val cueEmitter: (AssignmentCue) -> Unit = {},
) : AssignmentGateway {
    private val states = ConcurrentHashMap<String, WorkItemState>()
    private val sessionsByRootInputId = ConcurrentHashMap<String, AssignmentRunSession>()
    ...
    override fun nextWorkFromCue(cue: AssignmentCue): AssignmentActivation? {
        ...
        return withItemLock(itemId) {
            ...
            sessionsByRootInputId[rootInputId] = AssignmentRunSession(
            ...
            instrumentation.emit(AgentEvents.assignmentWakeUp(state.id, "work_ready", cue.reason))
    ...
    override fun beforeActionExecution(action: PendingAction): ActionExecutionGateDecision {
        if (action.origin.source != OriginSource.ASSIGNMENT) {
        ...
        if (workEffectLedger.isEffectCompleted(effectIntentId)) {
            return ActionExecutionGateDecision.deny(
                reason = "Skipped duplicate assignment side effect (${action.type.id}).",
    ...
    override fun onActionExecuted(action: PendingAction, outcome: ActionOutcome, observedEvidence: Boolean) {
        if (action.origin.source != OriginSource.ASSIGNMENT) return
    ...
    override fun allowFollowUp(action: PendingAction): Boolean {
        if (action.origin.source != OriginSource.ASSIGNMENT) return true
    ...
    override fun finalizeAssignmentCycle(rootInputId: String) {
    ...
    override fun notifyStepPlannerNoop(rootInputId: String, reason: String) {
        ...
        // If the step is still retryable (READY), set requeueReason so finalizeAssignmentCycle
    ...
    override fun executeOperation(request: AssignmentOperationRequest): AssignmentOperationResult {
        logger.info {
            "assignment_operation: op=${request.operation} workItemId='${request.workItemId.orEmpty()}' " +
        ...
        return when (request.operation) {
            AssignmentOperation.CREATE -> {
                ...
                if (instruction.isBlank()) {
                    AssignmentOperationResult(false, "Work item creation requires an instruction.")
                } else if (cronExpression.isNotBlank() && !isValidCronExpression(cronExpression)) {
                    AssignmentOperationResult(false, "Work item creation requires a valid 5-field cron_expression.")
                } else {
                    ...
                    if (workItemId.isBlank()) {
                        AssignmentOperationResult(false, "Work item creation was rejected.")
                    } else {
            ...
            AssignmentOperation.STATUS -> {
                val workItemId = request.workItemId.orEmpty()
                val state = states[workItemId]
                if (state == null) {
                    AssignmentOperationResult(false, "Work item not found.")
                } else {
            ...
            AssignmentOperation.REVIEW -> {
                ...
                if (state == null) {
                    ...
                    if (step == null) {
                        ...
                        AssignmentOperationResult(false, "Work item has no runnable step.")
                    } else {
                        if (request.reviewSource == ReviewRequestSource.ID) {
                            applyEvent(
                                workItemId,
                        ...
                        AssignmentOperationResult(true, "Work item review requested.", workItemId)
                    }
                }
            }
            ...
            AssignmentOperation.LIST -> {
                ...
                AssignmentOperationResult(true, message)
            }

            AssignmentOperation.PAUSE -> {
                val workItemId = request.workItemId.orEmpty()
                if (!states.containsKey(workItemId)) {
                    ...
                    AssignmentOperationResult(true, "Work item paused.", workItemId)
                }
            }

            AssignmentOperation.RESUME -> {
                val workItemId = request.workItemId.orEmpty()
                if (!states.containsKey(workItemId)) {
                    AssignmentOperationResult(false, "Work item not found.")
                } else {
                    applyEvent(workItemId, WorkItemEvent.Resumed(workItemId))
                    AssignmentOperationResult(true, "Work item resumed.", workItemId)
                }
            }

            AssignmentOperation.REPRIORITIZE -> {
                val workItemId = request.workItemId.orEmpty()
                val state = states[workItemId]
                val newPriority = request.priority
                if (state == null || newPriority == null) {
                    AssignmentOperationResult(false, "Work item reprioritize requires workItemId and priority.")
                } else {
                    applyEvent(workItemId, WorkItemEvent.PriorityChanged(workItemId, newPriority))
                    AssignmentOperationResult(true, "Work item priority updated to $newPriority.", workItemId)
                }
            }

            AssignmentOperation.COMPLETE -> {
                val workItemId = request.workItemId.orEmpty()
                if (!states.containsKey(workItemId)) {
                    AssignmentOperationResult(false, "Work item not found.")
                } else {
                    applyEvent(workItemId, WorkItemEvent.Completed(workItemId))
                    AssignmentOperationResult(true, "Work item marked completed.", workItemId)
                }
            }

            AssignmentOperation.RETIRE -> {
                val workItemId = request.workItemId.orEmpty()
                if (!states.containsKey(workItemId)) {
                    AssignmentOperationResult(false, "Work item not found.")
                } else {
                    ...
                    AssignmentOperationResult(true, "Work item retired.", workItemId)
                }
            }
            ...
            AssignmentOperation.DELETE -> {
                val workItemId = request.workItemId.orEmpty()
                if (workItemId.isBlank()) {
                    AssignmentOperationResult(false, "Work item delete requires workItemId.")
                } else if (!deleteWorkItemState(workItemId)) {
                    AssignmentOperationResult(false, "Work item not found.")
                } else {
                    AssignmentOperationResult(true, "Work item deleted.", workItemId)
                }
            }

            AssignmentOperation.DELETE_ALL -> {
            ...
            AssignmentOperation.REVISE_PLAN -> {
                val workItemId = request.workItemId.orEmpty()
                val state = states[workItemId]
                if (state == null) {
                    ...
                    if (plan == null) {
                        ...
                        return AssignmentOperationResult(false, "Work item plan revision rejected: invalid plan structure.")
                    }
                    applyEvent(
                        workItemId,
                    ...
                    AssignmentOperationResult(true, "Work item plan revised.", workItemId)
                } else {
            ...
            AssignmentOperation.UPDATE -> {
                ...
                if (workItemId.isBlank() || state == null) {
                    ...
                    AssignmentOperationResult(false, "Work item update requires a valid work_item_id. Work item not found.")
                } else {
                    val newCron = request.cronExpression?.trim()?.ifBlank { null }
                    if (newCron != null && !isValidCronExpression(newCron)) {
                        AssignmentOperationResult(false, "Invalid cron_expression: '$newCron'.")
                    } else {
                        applyEvent(
                            workItemId,
                        ...
                        AssignmentOperationResult(true, "Work item updated: $changes.", workItemId)
                    }
                }
            }
        }
    ...
    fun createWorkItem(
        ...
        planSteps: List<ai.neopsyke.agent.ego.planner.model.AssignmentPlanStepPayload>? = null,
    ): String {
        ...
        instrumentation.emit(AgentEvents.assignmentCreated(workItemId, title, priority.name))
    ...
    private fun buildValidatedPlanFromPayload(
        planSteps: List<ai.neopsyke.agent.ego.planner.model.AssignmentPlanStepPayload>,
    ...
    private fun applyEvent(workItemId: String, event: WorkItemEvent): WorkItemState? {
        ...
        if (oldStatus != updatedState.workItem.status) {
            instrumentation.emit(
                AgentEvents.assignmentStatusChanged(
                    workItemId,
                    oldStatus.name,
        ...
        when (event) {
            is WorkItemEvent.StepStarted -> {
                ...
                instrumentation.emit(
                    AgentEvents.assignmentStepStarted(workItemId, event.stepId, step?.description.orEmpty())
                )
            }
            ...
            is WorkItemEvent.StepAcceptancePassed -> {
                val step = updatedState.workItem.plan.steps.firstOrNull { it.id == event.stepId }
                instrumentation.emit(
                    AgentEvents.assignmentStepCompleted(workItemId, event.stepId, true, step?.attempts ?: 1)
                )
            }
            ...
            is WorkItemEvent.StepAcceptanceFailed -> {
                ...
                if (step?.status == StepStatus.FAILED) {
                    instrumentation.emit(
                        AgentEvents.assignmentStepCompleted(workItemId, event.stepId, false, step.attempts)
                    )
                }
            }
            ...
            is WorkItemEvent.StepBlocked -> {
                instrumentation.emit(
                    AgentEvents.assignmentBlocked(workItemId, event.stepId, event.waitCondition.type.name)
                )
            }

            is WorkItemEvent.WorkCycleCompleted -> {
                instrumentation.emit(
                    AgentEvents.assignmentCycleCompleted(workItemId, event.stepId, event.actionsExecuted)
                )
            }
        ...
        if (updatedState.isTerminal()) {
            instrumentation.emit(AgentEvents.assignmentCompleted(workItemId))
    ...
    private fun recordDeliveryOutcome(state: WorkItemState, session: AssignmentRunSession) {
    ...
    private fun writeWorkspaceCycleArtifacts(state: WorkItemState, session: AssignmentRunSession) {
    ...
    private fun classifyEffectClass(actionType: ActionType): EffectClass =
        when (actionType.id) {
            ...
            ActionType.ASSIGNMENT_OPERATION.id -> EffectClass.INTERNAL_STATEFUL
    ...
    private fun coalescePendingWake(workItemId: String, cue: AssignmentCue) {
        val state = states[workItemId] ?: return
        val pendingReasons = state.workItem.pendingWakeReasons
    ...
    companion object {
        ...
        private const val EFFECT_INTENT_DUPLICATE_REASON_CODE: String = "assignment_effect_duplicate"
        private const val EFFECT_INTENT_GATE_SOURCE: String = "assignment_effect_ledger"
