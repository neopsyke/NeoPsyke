...
class GoalWorkPlanner(
    ...
    override fun plan(trigger: EgoTrigger, context: PlannerContext): EgoDecision {
        ...
        if (TruncationRetry.isLikelyTruncated(response)) {
            ...
            runtime.call(laneId, allocation.messages + ChatMessage(ChatRole.USER, "Return one complete JSON object."),
                metadata.copy(callSite = "goal_work_truncation_retry"), MonolithicLaneStub.PLANNER_DECISION_RESPONSE_FORMAT, maxTokens = bumped, temperature = 0.0)
        ...
        runtime.call(laneId, allocation.messages + ChatMessage(ChatRole.USER, "Reply with STRICT JSON only."),
            metadata.copy(callSite = "goal_work_json_retry"), MonolithicLaneStub.PLANNER_DECISION_RESPONSE_FORMAT, temperature = 0.0)
            ?.let { parseResponse(it.content, context) }?.let { runtime.recordSuccess(laneId, rootInputId); return it }
    ...
    private fun parseResponse(raw: String, context: PlannerContext): EgoDecision? {
        ...
        return when (p.decision?.trim()?.lowercase()) {
            ...
            "plan" -> {
                val g = p.planGoal?.trim().orEmpty(); val st = p.planSteps?.map { it.trim() }?.filter { it.isNotBlank() }?.take(config.planner.maxPlanSteps)?.map { TextSecurity.clamp(it, config.planner.maxPlanStepDescriptionChars) }.orEmpty()
                if (g.isBlank() || st.isEmpty()) EgoDecision.Noop("Missing plan.") else EgoDecision.EnqueuePlan(Urgency.fromRaw(p.urgency), TextSecurity.clamp(g, config.planner.maxThoughtChars), st)
            }
