...
import ai.neopsyke.agent.ego.planner.model.DurableWorkDecision
...
/**
 * L1 lane: handles EgoTrigger.DurableWork with a goal-focused prompt.
 * Includes goal work summary, step details, acceptance criteria, available actions.
 */
class DurableWorkLanePlanner(
    ...
    override fun plan(trigger: EgoTrigger, context: PlannerContext): EgoDecision {
        val goalTrigger = trigger as? EgoTrigger.DurableWork
            ?: return EgoDecision.Noop("DurableWorkLanePlanner requires GoalWork trigger.")

        val rootInputId = goalTrigger.workUnit.workItemId
        ...
        val metadata = HierarchicalEgoPlanner.plannerChatMetadata(
            trigger = trigger, callSite = "durable_work",
        ...
        if (response == null) return EgoDecision.Noop("DurableWorkLanePlanner unavailable.")

        val parsedDecision = parseDurableWorkDecision(response.content, context)
        ...
        if (TruncationRetry.isLikelyTruncated(response)) {
            ...
            runtime.call(laneId, allocation.messages + ChatMessage(ChatRole.USER, "Return one complete JSON object."),
                ...
                ?.let { parseDurableWorkDecision(it.content, context) }?.let {
                    runtime.recordSuccess(laneId, rootInputId)
        ...
        instrumentation.emit(ai.neopsyke.instrumentation.AgentEvents.warning("DurableWorkLanePlanner response remained non-parseable after retry."))
        return EgoDecision.Noop(reason = "DurableWorkLanePlanner non-parseable.", parseFailureShortCircuit = runtime.isCircuitOpen(laneId, rootInputId))
    }

    private fun parseDurableWorkDecision(raw: String, context: PlannerContext): DurableWorkDecision? {
        val p = StructuredOutputHandler.parseWithRepair<DurableWorkPayload>(raw) { runtime.onOutputRepaired() } ?: return null
        return when (p.decision?.trim()?.lowercase()) {
            "intend" -> {
                ...
                if (ik == null || at == null || ap.isBlank() || s.isBlank()) {
                    DurableWorkDecision.FailStep("Invalid intention.")
                } else {
                    DurableWorkDecision.ExecuteStep(
                        ExecutionCandidate(
            ...
            "plan" -> {
                ...
                if (g.isBlank() || st.isEmpty()) {
                    DurableWorkDecision.FailStep("Missing plan.")
                } else {
                    DurableWorkDecision.RefinePlan(
                        urgency = Urgency.fromRaw(p.urgency),
                        goal = g,
                        steps = st,
                    )
            ...
            else -> DurableWorkDecision.FailStep(p.reason?.take(120) ?: "Noop.")
        }
    }

    private fun toEgoDecision(decision: DurableWorkDecision, context: PlannerContext): EgoDecision {
        return when (decision) {
            is DurableWorkDecision.ExecuteStep -> {
            ...
            is DurableWorkDecision.RefinePlan -> {
            ...
            is DurableWorkDecision.MarkStepComplete -> EgoDecision.Noop(decision.reason)
            is DurableWorkDecision.RequestClarification -> EgoDecision.FormIntention(
            ...
            is DurableWorkDecision.FailStep -> EgoDecision.Noop(decision.reason)
        }
    }

    private data class DurableWorkPayload(
