...
class WaitConditionMonitor(
    ...
    fun register(workItemId: String, stepId: String, condition: WaitCondition) {
        ...
        logger.debug { "Wait condition registered: goal=$workItemId, step=$stepId, type=${condition.type}" }
    }
    ...
    private suspend fun checkConditions() {
        ...
        for ((key, state) in snapshot) {
            ...
            if (timedOut) {
                logger.debug { "Wait condition timed out: goal=${key.workItemId}, step=${key.stepId}" }
