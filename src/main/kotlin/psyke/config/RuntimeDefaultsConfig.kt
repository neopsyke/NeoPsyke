...
data class RuntimeDefaultsConfig(
    ...
    val metricsDb: String = ".psyke/metrics.db",
)
...
object RuntimeDefaultsConfigLoader {
    ...
    fun resolveMetricsDbPath(
        ...
        env["PSYKE_METRICS_DB"]?.trim()?.takeIf { it.isNotEmpty() }?.let { explicit ->
            return expandPath(explicit, cwd)
        }

        val defaults = loadOrCreate(env, cwd)
        return expandPath(defaults.metricsDb.ifBlank { ".psyke/metrics.db" }, cwd)
    }
    ...
    private fun resolveConfigPath(env: Map<String, String>, cwd: Path): Path {
        val configured = env["PSYKE_RUNTIME_DEFAULTS_FILE"]?.trim().orEmpty()
        if (configured.isBlank()) {
            return cwd.resolve(".psyke").resolve("runtime-defaults.yaml")
        }
