...
class MistralWebSearchAgentSession private constructor(
    ...
    companion object {
        ...
        private fun createAgent(
            ...
            val payload = mapOf(
                "model" to model,
                "name" to "psyke-web-search-${UUID.randomUUID().toString().substring(0, 8)}",
                "description" to "Ephemeral web-search agent created by Psyke runtime.",
                "instructions" to "Use ${tool.apiValue} for up-to-date public web information.",
                "tools" to listOf(mapOf("type" to tool.apiValue)),
                "metadata" to mapOf("origin" to "psyke", "lifecycle" to "ephemeral")
            )
            val request = Request.Builder()
