...
class SensoryCortexTest {
    ...
    fun `notifyImpulseReady injects ImpulseReady signal into the channel`() = runBlocking {
        val scope = testScope()
        val source = psyke.agent.cortex.sensory.AsyncSignalSource(
            includeStdin = false,
        ...
        try {
            ...
            val signal = source.nextSignal()
            assertIs<psyke.agent.cortex.sensory.SystemSignal.ImpulseReady>(signal)
        } finally {
            source.close()
        }
    }
    ...
    fun `async stdin control-only mode ignores text input and only emits exit`() = runBlocking {
        ...
        val source = psyke.agent.cortex.sensory.AsyncSignalSource(
            ...
            emitStdinClosedSignal = false,
            pollTimeoutMs = 10L,
            stdinMode = psyke.agent.cortex.sensory.AsyncSignalSource.StdinMode.CONTROL_ONLY,
            readLineFn = {
