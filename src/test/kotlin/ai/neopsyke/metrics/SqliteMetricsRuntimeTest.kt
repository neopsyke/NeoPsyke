...
class SqliteMetricsRuntimeTest {
    ...
    fun `persistent totals are isolated by provider`() {
        val dbPath = Files.createTempFile("neopsyke-metrics-runtime-test", ".db")
        val previous = System.getProperty("neopsyke.metrics.db")
        System.setProperty("neopsyke.metrics.db", dbPath.toString())
        try {
            ...
            if (previous == null) {
                System.clearProperty("neopsyke.metrics.db")
            } else {
                System.setProperty("neopsyke.metrics.db", previous)
            }
    ...
    fun `response latency median is computed for odd and even sample counts`() {
        val dbPath = Files.createTempFile("neopsyke-metrics-latency-test", ".db")
        val previous = System.getProperty("neopsyke.metrics.db")
        System.setProperty("neopsyke.metrics.db", dbPath.toString())
        try {
            SqliteMetricsRuntime(
    ...
    fun `action call counters are tracked per run and aggregated for provider scope`() {
        val dbPath = Files.createTempFile("neopsyke-metrics-action-counts-test", ".db")
        val previous = System.getProperty("neopsyke.metrics.db")
        System.setProperty("neopsyke.metrics.db", dbPath.toString())
        try {
            SqliteMetricsRuntime(
    ...
    fun `snapshot includes token breakdowns by provider and role`() {
        val dbPath = Files.createTempFile("neopsyke-metrics-role-provider-breakdown-test", ".db")
        val previous = System.getProperty("neopsyke.metrics.db")
        System.setProperty("neopsyke.metrics.db", dbPath.toString())
        try {
            SqliteMetricsRuntime(
    ...
    fun `llmCallStats returns model stats with latency percentiles`() {
        val dbPath = Files.createTempFile("neopsyke-metrics-llm-stats-test", ".db")
        val previous = System.getProperty("neopsyke.metrics.db")
        System.setProperty("neopsyke.metrics.db", dbPath.toString())
        try {
            SqliteMetricsRuntime(
    ...
    fun `llmCallStats all-time scope aggregates across runs`() {
        val dbPath = Files.createTempFile("neopsyke-metrics-llm-stats-alltime-test", ".db")
        val previous = System.getProperty("neopsyke.metrics.db")
        System.setProperty("neopsyke.metrics.db", dbPath.toString())
        try {
            SqliteMetricsRuntime(
