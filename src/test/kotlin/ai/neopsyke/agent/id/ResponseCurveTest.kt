...
class ResponseCurveTest {
    ...
    fun `linear curve returns input unchanged`() {
        ...
        assertEquals(0.0, curve.urgency(0.0), epsilon)
        assertEquals(0.5, curve.urgency(0.5), epsilon)
        assertEquals(1.0, curve.urgency(1.0), epsilon)
    }

    @Test
    fun `linear curve clamps out-of-range values`() {
        val curve = ResponseCurve.Linear
        assertEquals(0.0, curve.urgency(-0.5), epsilon)
        assertEquals(1.0, curve.urgency(1.5), epsilon)
    }
    ...
    fun `power curve clamps out-of-range values`() {
        ...
        assertEquals(0.0, curve.urgency(-1.0), epsilon)
        assertEquals(1.0, curve.urgency(2.0), epsilon)
    }
    ...
    fun `sigmoid curve f(0) is approximately 0 and f(1) is approximately 1`() {
        ...
        assertTrue(curve.urgency(0.0) < 0.01, "sigmoid(0) should be ≈ 0")
        assertTrue(curve.urgency(1.0) > 0.99, "sigmoid(1) should be ≈ 1")
    }
    ...
    fun `sigmoid curve clamps out-of-range values`() {
        val curve = ResponseCurve.Sigmoid(steepness = 10.0, midpoint = 0.5)
        val low = curve.urgency(-1.0)
        val high = curve.urgency(2.0)
    ...
    fun `logarithmic curve f(0) is 0 and f(1) is approximately 1`() {
        ...
        assertEquals(0.0, curve.urgency(0.0), epsilon)
        assertTrue(curve.urgency(1.0) > 0.99, "log(1) should be ≈ 1")
    }

    @Test
    fun `logarithmic curve rises quickly from zero`() {
        val curve = ResponseCurve.Logarithmic(scale = 10.0)
        val lowValue = curve.urgency(0.1)
    ...
    fun `power dominates logarithmic at high values`() {
        ...
        val lowVal = 0.2
        val powerLow = power.urgency(lowVal)
        val logLow = log.urgency(lowVal)
        assertTrue(
            powerLow < logLow,
        ...
        val highVal = 0.95
        val powerHigh = power.urgency(highVal)
        val logHigh = log.urgency(highVal)
    ...
    fun `emergent hierarchy - logarithmic felt first, power felt last`() {
        ...
        val rawLow = 0.15
        assertTrue(
            log.urgency(rawLow) > sigmoid.urgency(rawLow),
            "At low value, logarithmic should dominate sigmoid"
        )
        assertTrue(
            log.urgency(rawLow) > power.urgency(rawLow),
    ...
    private fun assertMonotonicity(curve: ResponseCurve, steps: Int = 100) {
        var prev = curve.urgency(0.0)
        for (i in 1..steps) {
            val x = i.toDouble() / steps
            val current = curve.urgency(x)
            assertTrue(
