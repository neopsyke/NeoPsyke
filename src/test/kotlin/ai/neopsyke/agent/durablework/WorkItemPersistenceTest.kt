...
class ProjectPersistenceTest {
    ...
    fun `store falls back to snapshot when goal json is corrupt`() {
        ...
        try {
            ...
            store.saveSnapshot("proj-1", state)
            Files.writeString(root.resolve("proj-1").resolve(WorkItemStore.GOAL_FILE), "{not valid json")
            store.workItemEventLog("proj-1").append(
                WorkItemEvent.Created("proj-1", "Fallback Test", "recover from corrupt goal json",
