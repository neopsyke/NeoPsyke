...
private val providerServerLogger = KotlinLogging.logger {}

fun main(args: Array<String>) {
    val config = MemoryServerConfig.fromEnv()

    if (config.embeddingApiKey.isBlank()) {
        providerServerLogger.error { "EMBEDDING_API_KEY or MISTRAL_API_KEY is required. Set one and restart." }
        System.exit(1)
    }
    ...
    when (transport) {
        "http" -> HttpMemoryServer(host = host, port = port, runtime = runtime).use { server ->
            server.start()
            Thread.currentThread().join()
        }

        "mcp" -> McpMemoryServer(runtime).use { server ->
            server.run()
        }

        else -> {
            providerServerLogger.error { "Unsupported provider transport '$transport'. Expected http or mcp." }
            runtime.close()
            System.exit(1)
        }
    }
