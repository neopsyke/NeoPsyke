package ai.neopsyke.pgvector.memory.db

import ai.neopsyke.pgvector.memory.MemoryServerConfig
...
class MemoryRepositoryDbEvalTest {
    ...
    private fun withRepository(block: (MemoryRepository, MemoryServerConfig) -> Unit) {
    ...
    private fun resolveConfig(): MemoryServerConfig {
        val base = MemoryServerConfig.fromEnv()
    ...
    private fun detectExistingEmbeddingDimensions(config: MemoryServerConfig): Int? {
        return try {
